...
import Data.Char (isDigit, isAlphaNum, isSpace, toUpper, digitToInt)
...
nTimes' :: Int -> Parser a -> Parser [a]
nTimes' 0 _ = pure []
nTimes' n p = pure (:) <*> p <*> nTimes' (n-1) p
...
data Person 
  = MkPerson { first :: String, last :: String } 
  deriving Show

person :: Parser Person
person = pure MkPerson <*> (token (some alphaNum)) <*> (token (some alphaNum))
